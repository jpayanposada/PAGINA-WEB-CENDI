<style>
     .btn-info {
    background-color: #203563; 
    border-color: #203563; 
    color: #ffffff; 
  }

  .btn-info:hover {
    background-color:rgb(43, 70, 127); 
    border-color:rgb(43, 70, 127);
  }

  .btn.naranja:hover {
    background-color: #ffb333; 
    border-color: #ffb333;
  }

</style>

<section class="enroll-section spad set-bg" data-setbg="img/sliders/bannerBachillerato.png">
        <div class="container">
            <div class="row">

                <div class="col-lg-6">
                    <div class="section-title text-white">
                        <br>
                        <h3 style="text-align: center;" data-aos="fade-right" data-aos-duration="1000" data-aos-once="false">PLATAFORMA Q10 PARA BACHILLERATO</h3>
                        <p style="text-align: center;">Entérate de todo lo que te puede ofrecer</p>
                    </div>
                    <div class="enroll-list text-white">
                        <div class="enroll-list-item">
                            <span style="color: #f08006">1</span>
                            <h5>Acceso plataforma Q10</h5>
                            <a href="http://corporacioncendi.q10academico.com/" style="cursor: pointer;"><button type="button" class="btn btn naranja" style="color: white;">Ingresar ahora</button></a>
                        </div>
                        <div class="enroll-list-item">
                            <span style="color: #f08006;">2</span>
                            <h5>Pre_inscripciones - Colegio</h5>
                            <a href="https://site2.q10.com/Preinscripcion?aplentId=c866b1f7-b5a4-4146-a6d5-eda3de6bdea9" style="cursor: pointer;"><button type="button" class="btn btn naranja" style="color: white;">Ingresar
                                    ahora</button></a>
                        </div>
                        <div class="enroll-list-item">
                            <span style="color: #f08006;">3</span>
                            <h5>Ofertas Laborales - Colegio</h5>
                            <a href="https://site2.q10.com/Ofertalaboral/Crear?aplentId=c866b1f7-b5a4-4146-a6d5-eda3de6bdea9" style="cursor: pointer;"><button type="button" class="btn btn naranja" style="color: white;">Ingresar
                                    ahora</button></a>
                        </div>
                    </div>
                </div>

                <div class="col-lg-6">
                    <div class="section-title text-white">
                        <br>
                        <h3 style="text-align: center;" data-aos="fade-left" data-aos-duration="1000" data-aos-once="false">PLATAFORMA Q10 PARA <br>TÉCNICOS</h3>
                        <p style="text-align: center;">Entérate de todo lo que te puede ofrecer</p>
                    </div>
                    <div class="enroll-list text-white">
                        <div class="enroll-list-item">
                            <span style="color: #203563;">1</span>
                            <h5>Acceso plataforma Q10 técnicos</h5>
                            <a href="https://site3.q10.com/login?ReturnUrl=%2F&aplentId=d0e037d5-22c3-4082-b55e-d30628819936" style="cursor: pointer;"><button type="button" class="btn btn-info">Ingresar
                                    ahora</button></a>
                        </div>
                        <div class="enroll-list-item">
                            <span style="color: #203563;">2</span>
                            <h5>Pre_inscripciones - técnicos</h5>
                            <a href="https://site3.q10.com/Preinscripcion?aplentId=d0e037d5-22c3-4082-b55e-d30628819936" style="cursor: pointer;"><button type="button" class="btn btn-info" >Ingresar
                                    ahora</button></a>
                        </div>
                        <div class="enroll-list-item">
                            <span style="color: #203563;">3</span>
                            <h5>Ofertas Laborales - técnicos</h5>
                            <a href="https://site3.q10.com/Ofertalaboral/Crear?aplentId=d0e037d5-22c3-4082-b55e-d30628819936" style="cursor: pointer;"><button type="button" class="btn btn-info">Ingresar
                                    ahora</button></a>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </section>